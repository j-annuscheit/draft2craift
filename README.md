...
- `test_studio/main.py` dashboard for run comparison and label analysis
...
```bash
git clone https://github.com/j-annuscheit/draft2craift.git
cd draft2craift
python -m venv .venv
...
# RAG parameter sweep
python eval/rag_sweep.py \
  --suite eval/examples/rag_suite.example.json \
  --grid eval/examples/rag_sweep.example.json \
...
python testcase_studio/main.py --storage-dir runs/feedback

# Generate runnable suites from accepted feedback testcases
python eval/feedback_generate_tests.py \
