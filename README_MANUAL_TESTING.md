...
- Schreibrechte im Benutzerprofil (AppData/Home) und im Test-Projektordner vorhanden
...
- Lege eine eigene kleine und eine groessere PDF-Datei an.
- Lege eine eigene DOCX-Datei an.
...
- [ ] App starten.
- [ ] Pruefen: Menueleiste zeigt `Einstellungen` mit `Autosave-Projekt aktivieren`.
- [ ] Autosave EIN schalten (falls aus).
- [ ] Pruefen: Autosave-Verzeichnis wird angelegt, sobald erste Aenderung passiert.
...
- [ ] `README.md`, `data/welcome.md`, eigene `test.pdf`, eigene `test.docx` importieren.
...
Autosave-Basisverzeichnis (plattformabhaengig) ermitteln:

```bash
python - <<'PY'
from pathlib import Path
from PySide6.QtCore import QStandardPaths
raw = str(QStandardPaths.writableLocation(QStandardPaths.StandardLocation.AppDataLocation) or "").strip()
base = (Path(raw).expanduser() if raw else Path.home() / ".draft2craift").resolve(strict=False)
print((base / "autosave_project").resolve(strict=False))
PY
```

Nutze den ausgegebenen Pfad in den Schritten unten als `AUTOSAVE_DIR`.

- [ ] Zwei Draft-Tabs mit Inhalt anlegen.
- [ ] Shell: Zeitstempel notieren:
  - Linux/macOS: `ls -lah $AUTOSAVE_DIR/canvas`
  - Windows PowerShell: `Get-ChildItem $AUTOSAVE_DIR\\canvas`
...
- [ ] Erwartung: `AUTOSAVE_DIR` wird nicht weiter aktualisiert bzw. entfernt.
...
- Screenshot/Video
- Logauszug
- betroffene Datei (z. B. `AUTOSAVE_DIR/canvas/doc_0001.md`)
```
