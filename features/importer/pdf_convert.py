...
def convert_pdf_with_settings(
    ...
    settings: PDFImportSettings,
    *,
    parse_page_range: ParsePageRange,
    detect_pdf_hf_layout: DetectHfLayout,
    compute_body_font_size: ComputeBodyFontSize,
    custom_header_detector_factory: CustomHeaderDetectorFactory,
    reflow_markdown: ReflowMarkdown,
    ...
    try:
        pages = parse_page_range(settings.page_range, path)
    except Exception:
        pages = None
    ...
    try:
        ...
        _apply_hf_detection_state(path, settings, detect_pdf_hf_layout)
        hdr_info = _build_hdr_info(
            doc,
            settings,
            compute_body_font_size,
            custom_header_detector_factory,
        )
        ...
        page_entries, error = _convert_selected_pages(
            ...
            reflow_markdown,
            strip_bold_from_markdown_headings,
            recover_tables_in_page_markdown,
        )
        if error:
    ...
    return _finalize_pdf_markdown(
        ...
        page_entries,
        settings,
        merge_smart_page_boundaries,
        merge_table_page_boundaries,
        replace_html_br_with_space,
        limit_dot_leaders,
    )
