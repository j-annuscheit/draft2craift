...
from .viewer_panel import DocumentViewerPanel
from .files_panel import ImportedFilesPanel
from .rag_results.panel import RAGResultsPanel
...
class KnowledgeDock(QDockWidget):
    ...
    def get_imported_document_content(self, name: str) -> str:
        """Return markdown content for one imported document name."""
        return self.imported_files.get_document_content(name)

    def get_imported_documents(self) -> dict[str, str]:
        """Return a copy of all imported documents."""
        return self.imported_files.get_all_documents()
