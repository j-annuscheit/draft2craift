...
Stand der Doku: Codebasis vom 2026-02-28.
...
Es gibt sieben zentrale Eval-Typen in `scripts/`:

| Typ | Script | Ziel | GT (Ground Truth) |
|---|---|---|---|
| RAG Retrieval | `scripts/rag_eval.py` | Richtige Dokumente fuer Queries finden | Erwartete Dokumente + optionale erwartete Textfragmente |
| PDF -> Markdown | `scripts/pdf_eval.py` | PDF-Konvertierung gegen erwartetes Markdown pruefen | Erwartete Markdown-Datei |
| Glossary | `scripts/glossary_eval.py` | Zielbegriffe aus Markdown extrahieren | `target_terms` pro Case |
| MindMap/Graph | `scripts/mindmap_eval.py` | Gerenderten MindMap/Graph-View auf Pflicht-/Ausschluss-Strings pruefen | `must_contain` / `must_not_contain` pro Case |
| Fact-Check | `scripts/factcheck_eval.py` | Fakten extrahieren und/oder gegen Quellen verifizieren | GT-Fakten + GT-Verdikte |
| LLM-as-a-Judge | `scripts/judge_eval.py` | Zwischen zwei Kandidaten die bessere Antwort waehlen | Pro Case korrekter Gewinner (`A` oder `B`) |
| LLM-Compare (Judge) | `scripts/llm_compare_eval.py` | Zwei LLM-Settings gegeneinander vergleichen, bewertet durch Judge-LLM | Kein GT pro Case; Ergebnis ist Judge-Praeferenz |

Zusatz:

- `scripts/rag_sweep.py` macht Parameter-Sweeps fuer RAG.
- `scripts/test_studio.py` ist die Test-GUI fuer die bisherigen Kern-Typen.
- `scripts/testcase_studio.py` verwaltet Feedback -> Testcases (inkl. Accept/Reject).
...
## 4) RAG-Tests (`scripts/rag_eval.py`)
...
## 5) PDF-Tests (`scripts/pdf_eval.py`)
...
## 6) Glossary-Tests (`scripts/glossary_eval.py`)
...
## 7) Fact-Check-Tests (`scripts/factcheck_eval.py`)
...
- `scripts/examples/factcheck_suite.3stage.json`
...
## 7a) Judge-Tests (`scripts/judge_eval.py`)
...
## 7b) LLM-Compare-Tests (`scripts/llm_compare_eval.py`)
...
## 8) RAG Sweep (`scripts/rag_sweep.py`)
...
## 9) Test-GUI (`scripts/test_studio.py`)
...
- Suite-Default ist `scripts/examples/factcheck_suite.3stage.json`

## 10) Neue Test-Cases anlegen (Checkliste)

### 10.1 RAG

1. Neue Markdown-Quellen unter `scripts/examples/fixtures/...` ablegen.
...
```bash
python scripts/glossary_eval.py \
  --suite scripts/examples/glossary_suite.example.json \
...
```bash
python scripts/mindmap_eval.py \
  --suite scripts/examples/mindmap_suite.example.json \
...
```bash
python scripts/llm_compare_eval.py \
  --suite scripts/examples/llm_compare_suite.example.json \
